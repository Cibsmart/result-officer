...
<template>
  <RadioGroup v-model="theme"
    ...
    <RadioGroupOption v-slot="{ checked }" value="system">
      <span
        :class="checked ? 'bg-white inset-ring ring ring-gray-950/10 inset-ring-white/10 dark:bg-gray-700 dark:text-white dark:ring-transparent' : ''"
    ...
    <RadioGroupOption v-slot="{ checked }" value="light">
      <span
        :class="checked ? 'bg-white inset-ring ring ring-gray-950/10 inset-ring-white/10 dark:bg-gray-700 dark:text-white dark:ring-transparent' : ''">
        <svg class="size-6" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"
             xmlns="http://www.w3.org/2000/svg">
    ...
    <RadioGroupOption v-slot="{ checked }" value="dark">
      <span
        :class="checked ? 'bg-white inset-ring ring ring-gray-950/10 inset-ring-white/10 dark:bg-gray-700 dark:text-white dark:ring-transparent' : ''">
        <svg class="size-6" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"
             xmlns="http://www.w3.org/2000/svg">
