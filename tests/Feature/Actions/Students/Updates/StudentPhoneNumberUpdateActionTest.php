...
covers(StudentPhoneNumberUpdateAction::class);

it('updates student phone number', function (): void {
    $student = StudentFactory::new()->createOne();

    $newValue = '07012345678';

    (new StudentPhoneNumberUpdateAction())->execute($student, $newValue);

    $student->refresh();

    expect($student->phone_number)->toBe($newValue);
});
...
it('records the update in the student history table', function (): void {
    ...
    (new StudentPhoneNumberUpdateAction())->execute($student, $newValue, user: $user);
    ...
    assertDatabaseHas(StudentHistory::class, [
        ...
        'field' => StudentModifiableField::PHONE_NUMBER,
