...
final class ProcessLegacyFinalResults extends Command
{
    ...
    public function handle(): void
    {
        ...
        foreach ($legacyStudents as $legacyStudent) {
            ...
            $results = LegacyFinalResult::query()
    ...
    /** @param \Illuminate\Database\Eloquent\Collection<int, \App\Models\LegacyFinalResult> $results */
    ...
    /**
     * @param \Illuminate\Database\Eloquent\Collection<int, \App\Models\LegacyFinalResult> $sessionRegistrations
    ...
    /**
     * @param \Illuminate\Database\Eloquent\Collection<int, \App\Models\LegacyFinalResult> $semesterRegistrations
     * @param array<string, int> $courses
     */
    private function processSemesterEnrollments(
        ...
        foreach ($semesterRegistrations as $legacyFinalResult) {
            try {
                $course = Course::getUsingLegacyCourseId($legacyFinalResult->legacy_course_id);
                ...
                $dbRegistration = Registration::createFromLegacyResult(
                    $dbSemesterEnrollment,
                    $legacyFinalResult,
                    $status,
                );

                Result::createFromLegacyResult($dbRegistration, $legacyFinalResult);

                $legacyFinalResult->updateSuccess($dbRegistration);
            } catch (Exception $e) {
                $legacyFinalResult->updateFailure($e->getMessage());
            }
        }
    }
}
